...
export type EquipmentAction =
  'ACTIVATE' | 'OUT_OF_SERVICE' | 'START_MAINTENANCE' | 'RETURN_TO_SERVICE' | 'DECOMMISSION';
...
export function createDraftEquipment(input: NewEquipmentInput): Equipment {
  return {
    id: input.id,
    equipmentNo: required(input.equipmentNo, 'equipmentNo'),
...
export function updateDraftEquipment(
  equipment: Equipment,
  input: Pick<
    NewEquipmentInput,
...
const transitions: Record<EquipmentAction, readonly [EquipmentState, EquipmentState][]> = {
  ...
  OUT_OF_SERVICE: [['ACTIVE', 'OUT_OF_SERVICE']],
  START_MAINTENANCE: [
...
export function transitionEquipment(
  ...
  now: Date,
  reason?: string,
  activationPolicySatisfied = false,
): Equipment {
  ...
  if (['OUT_OF_SERVICE', 'DECOMMISSION'].includes(action) && !reason?.trim())
    throw new AppError('VALIDATION_FAILED', { userSafe: true });
  if (action === 'ACTIVATE' && !activationPolicySatisfied)
    throw new AppError('AUTHZ_DENIED', {
      userSafe: true,
      messageKey: 'assets.equipment.activationPolicyUnresolved',
    });
  const next = match[1];
  return {
    ...equipment,
    state: next,
    commissionedAt: action === 'ACTIVATE' ? now : equipment.commissionedAt,
    decommissionedAt: action === 'DECOMMISSION' ? now : equipment.decommissionedAt,
    updatedAt: now,
    version: equipment.version + 1n,
  };
}
