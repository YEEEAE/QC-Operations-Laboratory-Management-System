...
const entity = (x: CalibrationRecord) => ({
...
export class PostgresCalibrationRepository implements CalibrationRepository {
  ...
  async create(input: {
    ...
    try {
      const x = input.calibration;
      return await this.db.transaction().execute(async (tx) => {
        const row = await tx
          .insertInto('calibration_records')
          .values({
            id: x.id,
            calibration_no: x.calibrationNo,
            equipment_id: x.equipmentId,
            state: 'DRAFT',
            calibration_date: x.calibrationDate.toISOString().slice(0, 10),
            due_date: x.dueDate?.toISOString().slice(0, 10) ?? null,
            provider: x.provider ?? null,
            certificate_no: x.certificateNo ?? null,
        ...
        await this.auditFor(tx)?.append({
          ...
          subjectType: 'CALIBRATION_RECORD',
          subjectId: x.id,
        ...
        await this.outboxFor(tx)?.enqueue({
          ...
          aggregateId: x.id,
          payload: { calibrationNo: x.calibrationNo, equipmentId: x.equipmentId, state: 'DRAFT' },
          dedupeKey: `calibration-created:${x.id}`,
        });
        return map(row);
      });
    } catch (error) {
  ...
  async get(id: string, actor: ActorContext): Promise<CalibrationRecord | undefined> {
    ...
    const x = map(row);
    const grant = actor.permissions.find((p) => p.code === 'PERM-CAL-VIEW');
  ...
  async list(input: {
    ...
    if (input.filter?.search)
      query = query.where('calibration_no', 'ilike', `%${input.filter.search}%`) as typeof query;
    const rows = await query.execute();
    const grant = input.actor.permissions.find((p) => p.code === 'PERM-CAL-VIEW');
    return rows
      .map(map)
    ...
    try {
      ...
      return await this.db.transaction().execute(async (tx) => {
        ...
        const row = await tx
          ...
          .set({
            state: changed.state,
            approved_at: changed.approvedAt ?? null,
            approved_by: input.action === 'APPROVE' ? input.actor.id : (old.approvedBy ?? null),
            became_current_at: changed.becameCurrentAt ?? null,
            superseded_at: changed.supersededAt ?? null,
            voided_at: changed.voidedAt ?? null,
            void_reason: changed.voidReason ?? null,
        ...
        await this.outboxFor(tx)?.enqueue({
          ...
          dedupeKey: `calibration:${input.id}:v${changed.version}`,
        });
        return map(row);
      });
    } catch (error) {
