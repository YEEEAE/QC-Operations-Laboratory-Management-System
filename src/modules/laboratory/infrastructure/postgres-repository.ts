...
import type { LabRepository, Mutation } from '../ports/repository.js';
...
export class PostgresLabRepository implements LabRepository {
  ...
  async get(id: string, actor: ActorContext) {
    ...
    if (
      !actorHasScope(
        actor,
        { type: 'LAB_TEST', id, state: row.state, authorId: row.author_id },
        { ownerId: row.author_id },
        actor.permissions.find((p) => p.code === 'PERM-LAB-VIEW'),
      )
    )
      return undefined;
    const [parameters, samples, measurements, snapshot] = await Promise.all([
      this.db
        .selectFrom('lab_test_template_parameters')
        .selectAll()
        .where('template_version_id', '=', row.template_version_id)
        .execute(),
      this.db.selectFrom('lab_samples').selectAll().where('lab_test_id', '=', id).execute(),
      this.db.selectFrom('lab_measurements').selectAll().where('lab_test_id', '=', id).execute(),
      this.db
    ...
    return {
      ...
      samples: samples.map((s) => ({ id: s.id, identifier: s.sample_identifier })),
      measurements: measurements.map((m) => ({
        id: m.id,
        sampleId: m.sample_id!,
        parameterId: m.template_parameter_id,
        raw: m.raw_numeric_value ?? m.raw_text_value ?? m.raw_boolean_value!,
        unit: m.unit,
        remarks: m.remarks ?? undefined,
        enteredBy: m.entered_by,
        enteredAt: m.entered_at.toISOString(),
      })),
      context: {
        ...ctx.context,
        parameters: parameters.map((p) => ({
          ...
          label: p.label,
          dataType: p.data_type as 'NUMERIC' | 'TEXT' | 'BOOLEAN',
          unit: p.unit,
          required: p.required,
          sourceReference: p.controlled_source_reference ?? '',
          criteria: (p.acceptance_rule_payload ?? {}) as Readonly<Record<string, unknown>>,
        })),
      },
    };
  }
  async list(actor: ActorContext) {
    ...
    for (const row of rows) {
      const test = await this.get(row.id, actor);
