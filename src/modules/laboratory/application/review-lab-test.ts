...
export class ReviewLabTestUseCase {
  ...
  async execute(input: {
    ...
    authorizeLab(input.actor, test, 'PERM-LAB-REVIEW', 'REVIEW', input.expectedVersion, true);
    authorizeLab(input.actor, test, 'PERM-APR-REVIEW', 'REVIEW', input.expectedVersion, true);
    const at = this.now().toISOString();
    return this.repository.save(
      test,
      {
        ...test,
        state: transitionLab(test.state, 'REVIEW'),
        version: test.version + 1n,
        updatedAt: at,
        reviewStartedAt: at,
      },
      { actor: input.actor, requestId: input.requestId, action: 'REVIEW' },
    );
  }
}
