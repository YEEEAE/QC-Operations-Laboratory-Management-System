...
export class ResumeLabTestUseCase {
  constructor(
  ...
  async execute(input: {
    ...
    authorizeLab(input.actor, test, 'PERM-LAB-EDIT-DRAFT', 'RESUME', input.expectedVersion);
    const at = this.now().toISOString();
    return this.repository.save(
      test,
      {
        ...test,
        state: transitionLab(test.state, 'RESUME'),
        version: test.version + 1n,
        updatedAt: at,
      },
      { actor: input.actor, requestId: input.requestId, action: 'RESUME' },
    );
  }
}
