...
const attemptMap = (
  row: DatabaseRow<'change_application_attempts'>,
  ...
  ...(row.target_version_before !== null
    ? { targetVersionBefore: BigInt(row.target_version_before) }
    : {}),
  ...(row.target_version_after !== null
    ? { targetVersionAfter: BigInt(row.target_version_after) }
    : {}),
...
export class PostgresChangeRequestRepository implements ChangeRequestRepository {
  ...
  async get(input: {
    id: string;
    actor: ActorContext;
  }): Promise<ChangeRequestAggregate | undefined> {
  ...
  async list(input: {
    ...
    try {
      let query = this.scopedQuery(input.actor).selectAll().orderBy('updated_at', 'desc').orderBy('id', 'desc');
      if (input.filter?.state)
        query = query.where('state', '=', input.filter.state) as typeof query;
      if (input.filter?.targetType)
        query = query.where('target_type', '=', input.filter.targetType) as typeof query;
      if (input.filter?.requestedBy)
        query = query.where('requested_by', '=', input.filter.requestedBy) as typeof query;
  ...
  async findTransitionByRequestId(input: {
    ...
    const row = await this.database
      ...
      .where('subject_type', '=', 'CHANGE_REQUEST')
      .where('subject_id', '=', input.id)
      .where('request_id', '=', input.requestId)
      .where('action', 'like', 'CHANGE_REQUEST_%')
      .executeTakeFirst();
    ...
    try {
      return await this.database.transaction().execute(async (tx) => {
        const row = await tx
          ...
          .where('id', '=', input.id)
          .where('requested_by', '=', input.actor.id)
          .where('state', '=', 'DRAFT')
        ...
        await this.auditFor(tx)?.append({
          ...
          payload: {
            ...
            nextVersion: (input.expectedVersion + 1n).toString(),
          },
        });
        return this.loadAggregate(tx, requestMap(row));
      });
  ...
  async transition(input: {
    ...
    try {
      return await this.database.transaction().execute(async (tx) => {
        ...
        const next = transitionChangeRequest(requestMap(oldRow), {
          action: input.action,
          reason: input.reason,
          now: input.now,
        });
        ...
        await this.auditFor(tx)?.append({
          ...
          subjectType: 'CHANGE_REQUEST',
          subjectId: input.id,
          action: `CHANGE_REQUEST_${input.action}`,
          transitionId: `TR-CHG-${input.action}`,
          ...
          payload: {
            expectedVersion: input.expectedVersion.toString(),
            nextVersion: next.version.toString(),
          },
        });
        await this.outboxFor(tx)?.enqueue({
          eventType: 'CHANGE_REQUEST_TRANSITIONED',
          aggregateType: 'CHANGE_REQUEST',
          aggregateId: input.id,
          payload: { action: input.action, state: next.state, version: next.version.toString() },
      ...
      return await this.database.transaction().execute(async (tx) => {
        ...
        await tx
          ...
          .values({
            ...
            target_version_before: attempt.targetVersionBefore ?? null,
            target_version_after: attempt.targetVersionAfter ?? null,
  ...
  private scopedQuery(actor: ActorContext) {
    const workflow = actor.permissions.some(
      (permission) =>
    ...
    return this.database
      ...
      .where((eb) =>
        workflow
          ? eb.or([eb('requested_by', '=', actor.id), eb('target_type', 'is not', null)])
          : eb('requested_by', '=', actor.id),
      );
  }
  ...
  private async loadAggregate(
    ...
    const [changes, attempts, historyRows] = await Promise.all([
      database
        .selectFrom('change_request_changes')
        .selectAll()
    ...
    return {
      ...
      changes: changes.map(changeMap),
      history: historyRows.map((row) => historyMap(row)),
      applicationAttempts: attempts.map(attemptMap),
    };
  }
...
export function changeRequestCommandFingerprint(input: {
  action: ChangeRequestAction;
  expectedVersion: bigint;
  reason?: string;
}): string {
