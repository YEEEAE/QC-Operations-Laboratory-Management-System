...
export function createDraftTask(input: NewTaskInput): Task {
  const checklist = (input.checklist ?? []).map((item) => ({
    id: item.id,
    taskId: input.id,
    label: nonBlank(item.label, 'checklist'),
  ...
  return {
    ...
    priority: nonBlank(input.priority, 'priority'),
    state: 'DRAFT',
    dueAt: input.dueAt,
...
export function assertCompletable(task: Task): void {
  if (task.checklist.some((item) => item.required && !item.completed))
    throw new AppError('VALIDATION_FAILED', { userSafe: true });
  if (!task.requiredEvidencePresent || task.unresolvedMandatoryBlocker)
    throw new AppError('VALIDATION_FAILED', { userSafe: true });
}
