...
const permissions: Record<ReceivingAction, string> = {
  MARK_READY: 'PERM-QUAR-EDIT',
  START_INSPECTION: 'PERM-QUAR-START-INSPECTION',
  COMPLETE_INSPECTION: 'PERM-QUAR-EDIT',
  MOVE_TO_RELEASE_PENDING: 'PERM-QUAR-EDIT',
  RELEASE: 'PERM-QUAR-RELEASE',
  HOLD: 'PERM-QUAR-HOLD',
  REMOVE_HOLD: 'PERM-QUAR-HOLD',
  MARK_EXPIRED: 'PERM-QUAR-EDIT',
  CANCEL: 'PERM-QUAR-EDIT',
};

export interface TransitionReceivingInput {
  actor: ActorContext;
  id: string;
...
export class TransitionReceivingUseCase {
  ...
  async execute(input: TransitionReceivingInput) {
    ...
    authorize(
      {
        actor: input.actor,
        permission: permissions[input.action] as never,
        action: input.action,
        entity: {
          type: 'RECEIVING_ITEM',
          id: item.id,
