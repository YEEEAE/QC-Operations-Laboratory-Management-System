...
const map = (r: DatabaseRow<'sessions'>): Session => ({
  id: r.id as string,
  userId: r.user_id,
  tokenHash: r.session_token_hash,
  createdAt: r.created_at,
  ...(r.last_seen_at ? { lastSeenAt: r.last_seen_at } : {}),
  expiresAt: r.expires_at,
  ...(r.revoked_at ? { revokedAt: r.revoked_at } : {}),
  ...(r.revoked_reason ? { revokedReason: r.revoked_reason } : {}),
...
export class PostgresSessionRepository implements SessionRepository {
  ...
  async revokeAllForUser(userId: string, at: Date, reason: string) {
    await this.db
      .updateTable('sessions')
      .set({ revoked_at: at, revoked_reason: reason })
