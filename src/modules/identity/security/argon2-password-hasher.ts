...
function implementation(): Argon2Module {
  try {
    ...
    throw new AppError('SYSTEM_CONFIGURATION_INVALID', { cause, userSafe: false });
  }
}
...
export class Argon2idPasswordHasher implements PasswordHasher {
  async hash(password: string): Promise<string> {
    ...
    return implementation().hash(password, {
      type: 2,
      memoryCost: 19456,
      timeCost: 2,
      parallelism: 1,
  ...
  async verify(password: string, encodedHash: string): Promise<boolean> {
    ...
    try {
      return await implementation().verify(encodedHash, password);
    } catch {
      return false;
    }
  }
