...
export class CreateFindingUseCase {
  ...
  execute(i: {
    ...
    authorize(
      {
        actor: i.actor,
        permission: 'PERM-FIND-CREATE',
        action: 'CREATE',
        entity: { type: 'FINDING', id: 'new', state: 'DRAFT', ownerId: i.ownerId ?? i.actor.id },
    ...
    return this.repo.create({
      finding: createFinding({ id: uuidv7(), createdBy: i.actor.id, ...i, now: this.now() }),
      actor: i.actor,
