...
export function createCapa(
  ...
  if (!input.ncrId) throw new AppError('AUTHZ_DENIED', { userSafe: true });
  if (!input.title.trim() || !input.description.trim() || !input.actions.length)
...
export function transitionCapa(
  ...
  now: Date,
  reason?: string,
  conditions = { verified: false, effectivenessAccepted: false },
): Capa {
  const m: Record<CapaActionType, Partial<Record<CapaState, CapaState>>> = {
    OPEN: { DRAFT: 'OPEN' },
    START: { OPEN: 'IN_PROGRESS' },
    ACTIONS_COMPLETE: { IN_PROGRESS: 'AWAITING_VERIFICATION' },
    START_EFFECTIVENESS: { AWAITING_VERIFICATION: 'EFFECTIVENESS_REVIEW' },
    READY_FOR_CLOSURE: {
  ...
  const incomplete = c.actions.some((x) => x.state !== 'COMPLETED');
  if (
    !next ||
  ...
  return {
    ...c,
    state: next,
    closedAt: next === 'CLOSED' ? now : c.closedAt,
    updatedAt: now,
