...
export class UpdateRolePermissionsUseCase {
  ...
  async execute(input: {
    ...
    authorize(
      {
        actor: input.actor,
        permission: 'PERM-ADM-PERMISSION-ASSIGN',
        action: 'ASSIGN',
        entity: { type: 'ROLE', id: role.id, state: role.active ? 'ACTIVE' : 'INACTIVE' },
        scope: {},
        currentVersion: role.version,
        expectedVersion: input.expectedVersion,
        businessCondition: role.active,
      },
      { throwOnDeny: true },
    );
    return this.repository.replaceRolePermissions({ ...input, actorId: input.actor.id });
  }
}
