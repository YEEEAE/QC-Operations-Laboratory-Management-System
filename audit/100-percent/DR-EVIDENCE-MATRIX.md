...
**Posture:** evidence-first; current-HEAD logical restore drill `QC-CLOSURE-DR-008` is blocked by missing disposable PostgreSQL 18 runtime; repository checks are not a physical restore
...
|---|---|---|---|---|
| 1. Approved target | Isolated non-production target and change/incident authority | [`RESTORE-DRILL-RUNBOOK.md`](../../docs/operations/RESTORE-DRILL-RUNBOOK.md) defines isolation; QC-CLOSURE-DR-008 has no target because Docker/PostgreSQL 18 is unavailable | BLOCKED | Provide a disposable PostgreSQL 18 target and record non-secret target identity |
...
| 3. PostgreSQL context | Version, schema, core relations, history relations, and read-only connectivity | `scripts/recovery/validate-restored-database.ts`; current HEAD records local client 14.19 only | BLOCKED | Run checklist with the isolated PostgreSQL 18 `--database-url` |
| 4. Migration ledger | Exact ordered ledger and SHA-256 checksums match immutable repository migrations and application head `0023_uat_evidence` | Source ledger inventory generated for 23 migrations; no live ledger | UNVERIFIED | Capture `qc.schema_migrations` and compare; do not run forward migrations before baseline evidence |
...
| 8. Application compatibility | Exact release/Git SHA starts and reads restored data with matching migration head | Current build identity `rel-ebb1253bf3af841e` exists; no restored target | BLOCKED | Start exact release `54d4fd3…`/`0023_uat_evidence` and attach health/read-only record/file-link checks |
...
|---|---|---|
| 9 — Recovery | Gates 2–7 and a new evidence record | BLOCKED; current-HEAD record `RER-2026-09-17-008` has no runtime/artifact |
| 14 — Database recovery integrity | Gates 3–6, exact ledger/checksum comparison, no silent migration | BLOCKED/UNVERIFIED; source inventory exists but live comparison is absent |
| 20 — Evidence/file integrity | Gates 6–7, metadata linkage plus object size/hash | BLOCKED/UNVERIFIED; no object artifact or restored metadata |
| 82 — Restore verification | All gates 1–11, with isolated restore and exact release | BLOCKED; current-HEAD drill could not start |
