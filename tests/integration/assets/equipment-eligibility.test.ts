...
const context: ControlledContext = {
  ...
  equipment: [],
  source: {},
  requirementsReference: 'method',
};
const usage: EquipmentContext = {
  equipmentId: equipment.id,
  calibrationRecordId: calibration.id,
  usedAt: new Date().toISOString(),
  equipmentSnapshot: { equipmentId: equipment.id },
  calibrationSnapshot: { calibrationRecordId: calibration.id },
};
describe('Assets-owned laboratory eligibility', () => {
  it('accepts an active equipment/current calibration pair', async () => {
    const useCase = new GetEquipmentEligibilityUseCase(
      { getEquipment: async () => equipment, getCalibration: async () => calibration },
      () => new Date('2026-06-01T00:00:00Z'),
    );
    await expect(useCase.verify({ actor, equipment: [usage], context })).resolves.toBeUndefined();
  });
  it('denies overdue use while policy is unresolved', async () => {
    const useCase = new GetEquipmentEligibilityUseCase(
      {
