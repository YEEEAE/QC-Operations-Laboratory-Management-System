...
const grant = (
  code: PermissionGrant['code'],
  ...scopes: PermissionGrant['scopes']
): PermissionGrant => ({ code, scopes });
const actor = (id: string, permissions: PermissionGrant[], roles: string[] = []): ActorContext => ({
  id,
  accountState: 'ACTIVE',
  roles,
  permissions,
...
const record: ApprovalRecord = {
  approvalCase: {
    ...
    subjectId: ids.changeRequest,
    subjectVersion: 7n,
    workflowType: 'CHANGE_REQUEST_APPROVAL',
    state: 'IN_PROGRESS',
...
describe('Change Request approval authorization matrix', () => {
  it('requires both generic approval and PERM-CHG-APPROVE', () => {
    const genericOnly = actor(ids.approver, [
      grant('PERM-APR-VIEW-ASSIGNED', 'GLOBAL'),
      grant('PERM-APR-APPROVE', 'GLOBAL'),
    ]);
    assertCode(
      () => authorizeApprovalDecision(record, genericOnly, 'APPROVE', 7n),
      'AUTHZ_PERMISSION_MISSING',
    );

    const domainOnly = actor(ids.approver, [
      grant('PERM-APR-VIEW-ASSIGNED', 'GLOBAL'),
      grant('PERM-CHG-APPROVE', 'GLOBAL'),
    ]);
    assertCode(
      () => authorizeApprovalDecision(record, domainOnly, 'APPROVE', 7n),
      'AUTHZ_PERMISSION_MISSING',
    );

    const correct = actor(ids.approver, [
      grant('PERM-APR-VIEW-ASSIGNED', 'GLOBAL'),
      grant('PERM-APR-APPROVE', 'GLOBAL'),
      grant('PERM-CHG-APPROVE', 'GLOBAL'),
    ]);
  ...
  it('denies self-approval, stale subject versions, and an Admin role without grants', () => {
    const self = actor(ids.requester, [
      grant('PERM-APR-VIEW-ASSIGNED', 'GLOBAL'),
      grant('PERM-APR-APPROVE', 'GLOBAL'),
      grant('PERM-CHG-APPROVE', 'GLOBAL'),
    ]);
    const selfRecord = {
      ...record,
      workItem: { ...record.workItem, assignedUserId: ids.requester },
    };
    assertCode(
      () => authorizeApprovalDecision(selfRecord, self, 'APPROVE', 7n),
      'AUTHZ_SOD_VIOLATION',
    );
    ...
    assertCode(
      () => authorizeApprovalDecision(record, correct, 'APPROVE', 6n),
      'CONFLICT_STALE_VERSION',
    );

    const admin = actor(ids.approver, [grant('PERM-ADM-PERMISSION-ASSIGN', 'GLOBAL')], ['Admin']);
    assertCode(
      () => authorizeApprovalDecision(record, admin, 'APPROVE', 7n),
      'AUTHZ_PERMISSION_MISSING',
    );
  });

  it('requires an explicit generic/domain pair for return and rejection too', () => {
    const actorWithReturn = actor(ids.approver, [
      grant('PERM-APR-VIEW-ASSIGNED', 'GLOBAL'),
      grant('PERM-APR-RETURN', 'GLOBAL'),
      grant('PERM-CHG-RETURN', 'GLOBAL'),
    ]);
    expect(() => authorizeApprovalDecision(record, actorWithReturn, 'RETURN', 7n)).not.toThrow();
    const missingDomain = actor(ids.approver, [
      grant('PERM-APR-VIEW-ASSIGNED', 'GLOBAL'),
      grant('PERM-APR-REJECT', 'GLOBAL'),
    ]);
    assertCode(
      () => authorizeApprovalDecision(record, missingDomain, 'REJECT', 7n),
      'AUTHZ_PERMISSION_MISSING',
    );
  });
});
