      ...
      `SELECT table_name FROM information_schema.tables
       WHERE table_schema = 'qc'
         AND table_name IN (
           ...
           'equipment', 'calibration_records', 'maintenance_records', 'document_identities',
           'document_versions', 'document_version_files', 'approval_cases', 'approval_work_items',
           'approval_decisions', 'electronic_signatures', 'change_requests', 'change_request_changes',
           'change_application_attempts', 'backup_runs', 'restore_runs'
         ) ORDER BY table_name`,
    );
