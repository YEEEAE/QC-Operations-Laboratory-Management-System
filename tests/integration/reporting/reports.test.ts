...
describe('report registry and canonical datasets', () => {
  it('rejects an unknown report code', () => {
    expect(() => new ReportRegistry().get('arbitrary-table')).toThrowError(
  ...
  it('passes the authenticated actor and server-validated filters to the canonical query', async () => {
    ...
    const query: ReportQuery = {
      run: async (definition, currentActor, filters) => {
        ...
        return { definition, columns: definition.columns, rows: [{ receivingNo: 'R-1' }] };
      },
    };
    const result = await new RunReportUseCase(new ReportRegistry(), query).execute(
      actor(),
      'quarantine-aging',
      { from: '2026-01-01' },
    );
  ...
  it('denies report execution without the explicit run permission', async () => {
    ...
    await expect(
      new RunReportUseCase(new ReportRegistry(), query).execute(
        actor([{ code: 'PERM-RPT-VIEW', scopes: ['OWN'] }]),
        'quarantine-aging',
        {},
      ),
