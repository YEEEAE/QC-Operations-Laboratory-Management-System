...
const repository = (overrides: Partial<AuthorizationRepository> = {}): AuthorizationRepository =>
  ({
    ...
    replaceRolePermissions: vi.fn().mockResolvedValue({ ...role, version: 3n }),
    listUserScopes: vi.fn(),
...
describe('authorization administration use cases', () => {
  ...
  it('blocks self scope grant even with explicit permission', async () => {
    await expect(
      new ManageUserScopesUseCase(repository()).execute({
        actor: actor([{ code: 'PERM-ADM-SCOPE-ASSIGN', scopes: ['GLOBAL'] }]),
        userId: 'actor',
        scopes: [{ kind: 'SITE', value: 'site-1' }],
        requestId: 'r1',
      }),
