...
const inspection = (state: Inspection['state'] = 'SUBMITTED'): Inspection => ({
  id: '01900000-0000-7000-8000-000000000003',
  inspectionNo: 'INSP-1',
  receiving: {
    receivingId: '01900000-0000-7000-8000-000000000004',
    receivingNo: 'RCV-1',
    docNo: 'DOC-1',
    itemCode: 'ITEM-1',
...
const actor = (id: string): ActorContext => ({
  ...
  roles: ['SUPERVISOR'],
  permissions: [
    { code: 'PERM-INSP-REVIEW', scopes: ['GLOBAL'] },
...
describe('Quarantine inspection review and approval', () => {
  it('requires distinct review and approval permissions and blocks self-review', async () => {
    ...
    await expect(
      new ReviewInspectionUseCase(repo).execute({
  ...
  it('returns with a required reason and preserves the controlled path', async () => {
    ...
    await expect(
      new ReturnInspectionUseCase(repo).execute({
        actor: actor(reviewerId),
        id: inspection().id,
        expectedVersion: 4n,
