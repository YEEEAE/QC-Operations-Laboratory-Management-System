...
const capa: Capa = {
  ...
  actions: [
    {
      id: 'a1',
      capaId: '00000000-0000-0000-0000-000000000001',
      sequenceNo: 1,
      description: 'Action',
      ownerId: 'u1',
      state: 'OPEN',
...
const makeRepo = (): CapaRepository & { close: ReturnType<typeof vi.fn> } => ({
  ...
  list: vi.fn(),
  transition: vi.fn(),
  close: vi.fn(async () => ({ ...capa, state: 'CLOSED' as const, version: 2n })),
});
...
describe('CloseCapaUseCase', () => {
  it('closes an eligible CAPA despite incomplete actions/effectiveness', async () => {
    const repo = makeRepo();
    const result = await new CloseCapaUseCase(repo, verifier).execute({
      actor: actor(),
      id: capa.id,
      expectedVersion: 1n,
      reason: 'Approved P-04 exception',
  ...
  it.each([
    ...
    ['Manager', ['MANAGER']],
    ['Admin', ['ADMIN']],
    ['yazeed without Supervisor', ['SYSTEM_OWNER']],
  ])('%s is denied', async (_name, roles) => {
    const repo = makeRepo();
    await expect(
      new CloseCapaUseCase(repo, verifier).execute({
        actor: actor(roles),
        id: capa.id,
        expectedVersion: 1n,
        reason: 'reason',
  ...
  it('denies missing permission, inactive account, failed reauth, and missing reason', async () => {
    ...
    await expect(
      new CloseCapaUseCase(repo, verifier).execute({
        actor: { ...actor(), accountState: 'INACTIVE' },
        id: capa.id,
        expectedVersion: 1n,
        reason: 'reason',
        reauthenticationSecret: 'secret',
