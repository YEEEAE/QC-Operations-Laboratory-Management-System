...
- Exact current HEAD: `298e307721af97d9c1bd22279d0c784fbf5b62a8` on `main` (verified 2026-09-18, dirty working tree; user changes preserved). This exact commit is the one bound by the live Render deployment record (`dep-damfhv8u01pc738s4430`, `live`, finished `2026-09-18T08:43:07Z`), so source and deployed SHA now agree. Migration source head is `0029_performance_query_indexes`; it is still not applied to Render and was not runtime-applied against Render during this task. A controlled disposable PostgreSQL 18.6 run of the canonical chain reached head `0029` with 29 migrations / 77 tables / 0 orphans and a zero-op re-run.
...
- Deterministic dataset is `qc-ai-governance-v2` / `2.0.0`; the focused provider/advisory/security run remains `53/53 PASS` across 4 files. The current full unit run is `83 files / 564 PASS`; the current local security command is `52/52 PASS`. Typecheck, architecture, and build pass under local Node `v22.22.3` (outside the declared Node contract); format and lint do not fully pass.
