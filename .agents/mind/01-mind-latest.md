...
- **QC-100-FINAL-004 human UAT + sign-off (2026-09-19):** `BLOCKED` — no real participants, no approved staging/UAT environment, no frozen release candidate, UAT-DD-001 signer unresolved; no `SIGNED_UAT_CYCLE` exists and none can be fabricated (release-gate evidence is server-derived, fail-closed, candidate-bound). Former finding (now superseded): the codebase had **no writer path** for `qc.uat_*` tables or `release_gate_evidence(source='SIGNED_UAT_CYCLE')`. That path now exists and is **verified locally**: `src/modules/uat-evidence/` + `scripts/uat/ingest-uat-evidence.ts` (`pnpm uat:ingest`) pass their PostgreSQL suite 8/8 on a disposable PG 18.6 database. The safety property is intact and reproduced live: an automated-only cycle is refused, so no synthetic acceptance and no `SIGNED_UAT_CYCLE` row exist anywhere; the `uat` gate remains `UNVERIFIED` because no real human session or sign-off exists. Local UAT environment is brought up (Task 6). Evidence: `audit/2026-09-19/QC-100-FINAL-004-human-uat-signoff.md`, `audit/2026-09-19/QC-100-FINAL-004-task5-uat-ingestion-closure.md`, `audit/2026-09-19/QC-100-FINAL-004-task6-local-uat-bringup.md`.
...
- **2026-09-19 — QC-100-FINAL-004 / Task 3: Reject Issue Slip checkpoint order gating (candidate-side)**
  - Changed: تأكيدات نقاط Issue Slip صارت إلزامية التسلسل `SUPERVISOR→QC_MANAGER→FACTORY_DIRECTOR` في الـdomain والـuse case (`nextPendingApprovalRole`/`assertApprovalRoleIsNext`) مع إبقاء عقد «إقرار يسجّله المنشئ»؛ لا يجوز لمنشئ EMPLOYEE تأكيد نقطة أعلى من النقطة الحالية، والقاعدة لا تتجاوزها أي دور (QCM أيضًا)؛ صفحة التفاصيل تعرض زر التأكيد للنقطة التالية فقط وتشرح السبب للنقاط غير التالية. لا تغيير schema/migration.
  - Evidence: unit `2 files / 20 PASS` (سلبيات EMPLOYEE→QC_MANAGER/FACTORY_DIRECTOR، QCM→QC_MANAGER قبل SUPERVISOR، تكرار النقطة المؤكدة، غير المنشئ، VOID، ومسارات REVERSED)؛ integration على PostgreSQL 18.6 مصرفي محلي `tests/integration/reject-reports/reject-reports.test.ts` `6/6 PASS` (السلسلة المرتبة كاملة ما زالت تمر)؛ architecture PASS؛ prettier/eslint PASS على المسارات المعدّلة. **typecheck على HEAD: 9 أخطاء قائمة لا يمسّها هذا الـdiff** (`src/modules/laboratory/application/dependencies.ts` duplicate identifiers، و`src/pages/quarantine/inspections/[inspectionId]/review.astro` `canApprove`) — أثر Task 2 غير المُغلق في الكوميت الحالي، ويجب تسويته قبل بوابات Task 8.
  - State: DONE (مرشّح محليًا) / PARTIAL للـE2E المصادق عليه وUAT والنشر. Key files: `src/modules/reject-reports/domain/issue-slip.ts`, `src/modules/reject-reports/application/{confirm-issue-slip-approval,view-model}.ts`, `src/pages/reject-reports/issue-slips/[reportId].astro`, `tests/unit/reject-reports/{domain,approval-order}.test.ts`, `Documents/REJECT-REPORTS.md`.
...
- **2026-09-19 — توحيد الوثائق في `Documents/`**
  - Changed: شجرة `docs/` كاملة (development/operations/architecture/REJECT-REPORTS وغيرها) نُقلت إلى `Documents/` بشكل مسطّح؛ `docs/superpowers/plans|specs` حُذفت؛ الجرد `Documents/DOCUMENTATION-INVENTORY.md` حُدّث بالمسارات الجديدة. أي مسار قديم بصيغة `docs/...` في هذه الذاكرة تاريخي.
  - Evidence: `git status` يظهر الحذف من `docs/` ووجود الملفات في `Documents/`؛ الجرد محدّث ومُراجَع.
  - State: DONE. Key files: `Documents/DOCUMENTATION-INVENTORY.md`.
