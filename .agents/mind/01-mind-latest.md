...
- **QC-100-FINAL-004 human UAT + sign-off (2026-09-19):** `BLOCKED` — no real participants, no approved staging/UAT environment, no frozen release candidate, UAT-DD-001 signer unresolved; no `SIGNED_UAT_CYCLE` exists and none can be fabricated (release-gate evidence is server-derived, fail-closed, candidate-bound). Former finding (now superseded): the codebase had **no writer path** for `qc.uat_*` tables or `release_gate_evidence(source='SIGNED_UAT_CYCLE')`; that path now exists and is verified locally (Task 5), and **no** synthetic acceptance or `SIGNED_UAT_CYCLE` row exists anywhere, so the `uat` gate remains `UNVERIFIED`. Local UAT environment is brought up (Task 6) and an automated HTTP scenario suite runs green on it (Task 7, which can never flip the gate). Evidence: `audit/2026-09-19/QC-100-FINAL-004-{human-uat-signoff,task5-uat-ingestion-closure,task6-local-uat-bringup,task7-uat-scenarios}.md`.
